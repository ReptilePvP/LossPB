...
void showWiFiKeyboard() {
    if (wifi_keyboard == nullptr) {
        ...
        // Configure button matrix spacing and dimensions for larger buttons by reducing padding
...
void connectToWiFi() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        // Configure NTP time synchronization if not already synced
        if (!ntpTimeSynced) {
            configTime(GMT_OFFSET_SEC, DAYLIGHT_OFFSET_SEC, NTP_SERVER1, NTP_SERVER2);
            DEBUG_PRINT("NTP time synchronization configured");
            ...
            if (timeinfo.tm_year >= (2020 - 1900)) {
                DEBUG_PRINT("Time synchronized successfully");
                char time_buffer[30];
                strftime(time_buffer, sizeof(time_buffer), "%Y-%m-%d %H:%M:%S", &timeinfo);
                DEBUG_PRINTF("Current time: %s", time_buffer);
                ntpTimeSynced = true;
            } else {
                DEBUG_PRINT("Failed to get NTP time");
            }
        }
{{ ... }}
void connectToSavedNetworks() {
    ...
    if (WiFi.status() == WL_CONNECTED && !ntpTimeSynced) {
        ...
        if (timeinfo.tm_year >= (2020 - 1900)) {
            ...
            DEBUG_PRINTF("Current time: %s", time_buffer);
            ntpTimeSynced = true;
            playSuccessSound(); // Play success sound only once when time is synced
        } else {
            DEBUG_PRINT("Failed to get NTP time");
        }
    }
...
void playButtonSound() {
    if (AUDIO_ENABLED) {
        CoreS3.Speaker.tone(BUTTON_BEEP_FREQ, BUTTON_BEEP_DURATION);
    }
}

void playSuccessSound() {
    if (AUDIO_ENABLED) {
        CoreS3.Speaker.tone(SUCCESS_BEEP_FREQ, SUCCESS_BEEP_DURATION);
        delay(SUCCESS_BEEP_DURATION + 50);
        CoreS3.Speaker.tone(SUCCESS_BEEP_FREQ, SUCCESS_BEEP_DURATION);
    }
}
...
void playErrorSound() {
    if (AUDIO_ENABLED) {
        CoreS3.Speaker.tone(ERROR_BEEP_FREQ, ERROR_BEEP_DURATION);
        delay(ERROR_BEEP_DURATION + 50);
        CoreS3.Speaker.tone(ERROR_BEEP_FREQ, ERROR_BEEP_DURATION);
    }
}
...
void playNotificationSound() {
    if (AUDIO_ENABLED) {
        CoreS3.Speaker.tone(NOTIFICATION_BEEP_FREQ, NOTIFICATION_BEEP_DURATION);
    }
}
